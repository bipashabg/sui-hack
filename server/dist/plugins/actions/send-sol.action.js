...
import { Connection, SystemProgram, LAMPORTS_PER_SOL, Transaction, PublicKey, clusterApiUrl, } from "@solana/web3.js";
...
export class SendSOLAction extends CollabLandBaseAction {
    constructor() {
        ...
        const description = "Extracts the chain, amount of SOL to send, and the account to send to from the recent messages, which the user has requested to send SOL to.";
        const handler = async (_runtime, _message, _state, _options, _callback) => {
            try {
                ...
                const balance = await new CollabLandSolanaWalletBalanceProvider().get(_runtime, _message, _state);
                ...
                const amount = Math.floor(LAMPORTS_PER_SOL * parseFloat(extractedFundData.amount));
                ...
                console.log("[SendSOLAction] transaction.feePayer:", transaction.feePayer);
                transaction.recentBlockhash = (await connection.getLatestBlockhash()).blockhash;
                console.log("[SendSOLAction] transaction.recentBlockhash", transaction.recentBlockhash);
                const serializedTransaction = transaction
                    ...
                    .toString("base64");
                console.log("[SendSOLAction] serializedTransaction", serializedTransaction);
                const payload = {
                    serializedTransactionBase64: serializedTransaction,
                };
                console.log("[SendSOLAction] payload", payload);
                const { data: _resData } = await this.client.post(`/telegrambot/solana/submitTransaction?network=${chainId}`, payload, {
                    headers: {
                ...
                console.log("Hitting Collab.Land APIs for confirming Solana transaction...");
                const { data: _txReceiptData } = await this.client.get(`/telegrambot/solana/transactionResponse?network=${chainId}&txSignatureBase64=${_resData.txSignature}`, {
                    headers: {
                        "Content-Type": "application/json",
                        "X-TG-BOT-TOKEN": process.env.TELEGRAM_BOT_TOKEN,
                        "X-API-KEY": process.env.COLLABLAND_API_KEY,
                ...
                console.log("[SendSOLAction] response from Collab.Land API", _txReceiptData);
...
//# sourceMappingURL=send-sol.action.js.map