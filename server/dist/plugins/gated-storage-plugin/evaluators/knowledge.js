import { elizaLogger, ModelClass, generateText, } from "@ai16z/eliza";
import { StorageService } from "../services/storage.service.js";
export const knowledgeEvaluator = {
    ...
    The following is the memory content you need to evaluate: ${memory.content.text}`;
        ...
        if (important) {
            elizaLogger.log(`[knowledge handler] Important content found in memory. Storing message with embedding`);
            const { content, embedding } = memory;
            const storageService = StorageService.getInstance();
            await storageService.start();
            const doc = await storageService.storeMessageWithEmbedding(content.text, embedding, true);
            if (!doc) {
                return;
            }
            if (state) {
            ...
            elizaLogger.debug(`[knowledge handler] Stored message with embedding with stream ID ${doc.id}`);
        }
        else {
            elizaLogger.debug("[knowledge handler] No important content found in memory.");
        }
        return;
    },
    name: "knowledgeEvaluator",
    validate: async (_runtime, memory, state) => {
        if (StorageService.getInstance().isConfigured() &&
            !(state === null || state === void 0 ? void 0 : state.hasGatedAndStored)) {
