...
router.post("/init", async (_req, res) => {
    try {
        ...
        authUrl.searchParams.set("redirect_uri", `${ngrokURL}/auth/discord/callback`);
...
router.get("/callback", async (req, res) => {
    try {
        ...
        states.delete(state);
        return res.redirect(302, `${ngrokURL}/auth/discord/success?discord_token=${tokenResponse.data.access_token}`);
    }
    catch (error) {
        const ngrokURL = await NgrokService.getInstance().getUrl();
