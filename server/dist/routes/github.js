...
router.post("/init", async (_req, res) => {
    try {
        ...
        authUrl.searchParams.set("redirect_uri", `${ngrokURL}/auth/github/callback`);
...
router.get("/callback", async (req, res) => {
    try {
        ...
        states.delete(state);
        return res.redirect(302, `${ngrokURL}/auth/github/success?github_token=${tokenResponse.data.access_token}`);
    }
    catch (error) {
        const ngrokURL = await NgrokService.getInstance().getUrl();
